...
package biosyndesign.core.utils.svg2xml;
		...
		for (int i = 0; i < gui.sourceFiles.length; i++)
		{
			...
			if (groupConfigString != null)
			{
				...
				if (groupConfigDoc.getElementsByTagName("shape").getLength() > 0)
				{
					...
					{
						destConfigDoc.setAspect(XmlConfig.aspectType.FIXED);
					}
					else
					{
						destConfigDoc.setAspect(XmlConfig.aspectType.VARIABLE);
					}
...
//						Svg2Xml.printDocument(srcSVGDoc, System.out);
			...
			try
			{
				...
				if (isNewGroup)
				{
					...
					groupXml = "<shapes name=\"" + "compound_shapes" + "\">" + System.getProperty("line.separator");
